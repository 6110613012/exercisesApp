...
class HistoryStore: ObservableObject {
    ...
    func addDoneExercise(_ exerciseName: String) {
        ...
        if let firstDate = exerciseDays.first?.date, today.isSameDay(as: firstDate) {
            if exerciseDays[0].exercises.contains(exerciseName){print("1")}
            else{exerciseDays[0].exercises.append(exerciseName)}
        } else {
            exerciseDays.insert(
    ...
    func getURL() -> URL? {
        ...
        return documentsURL.appendingPathComponent("histroy.plist")
    }
}
